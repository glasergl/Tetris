...
public abstract class TetrisShape {
    ...
    protected TetrisShape(final TetrisGame whereThisShouldSpawn, final Color ofThis, final TetrisLocation start, final Direction facingFirst, final int width, final int height) {
     ...
     */
    public abstract Set<TetrisLocation> getCoveredLocations(final TetrisLocation ifThisIsHere, final Direction ifThisFacesThat);
	...
	if (current.getColumn() + height - 1 >= TetrisGame.NUMBER_OF_COLUMNS || current.getRow() + width - 1 >= TetrisGame.NUMBER_OF_ROWS) {
	    return false;
	} else {
	...
	return !whereThisShouldSpawn.isCovered(getCoveredLocations(ifThisWasHere, ifThisFacesThis), getCoveredLocations());
    }
